package com.pwd.mavenTest.util.string;
...
import com.pwd.mavenTest.util.datastruc.LinkedStack;
import com.pwd.mavenTest.util.logic.LogicUtil;
...
public final class StringUtil {
	...
	/**
	 * 格式化打印日志时的参数, dataKey和dataValue的长度必须一致
	 * @author hewenmin
	 * @date 2015年12月31日 下午1:57:11 
	 *
	 * @param dataKey 格式化参数的key
	 * @param dataValue 名字
	 * @return
	 */
	public static String formatLogger(String[] dataKey, Object[] dataValue){
		...
		try {
			if(dataKey == null || dataValue == null){
				return "";
			}else if(dataKey.length != dataValue.length){
				return "";
			}
	...
	public static String abandonStartEndStr(String str) {
		if (str == null || "null".equals(str) || "".equals(str) || String.valueOf(str).length() < 2) {
			return "";
		} else {
